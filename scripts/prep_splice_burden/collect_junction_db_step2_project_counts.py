...
with h5py.File(sbam, 'r') as IN:
    for key in IN:
        if key.endswith('_introns_m'):
            ...
            continue
        chrm = key.split('_')[0]
        junc_quant = np.zeros((len(junctions[chrm, strand]), ), dtype='int')

        #qidx = IN[key][:, 2] >= 3

        qidx = np.where([(_[0], _[1]) in junctions[chrm, strand] for _ in IN[key][:]])[0]
        if np.sum(qidx) == 0:
            outkey = 'junctions_%s_%s' % (chrm, STRANDS[strand])
            OUT.create_dataset(name=outkey, data=junc_quant[:, np.newaxis], compression='gzip', maxshape=(junc_quant.shape[0], None)) 
            continue

        idxs = np.array([junctions[chrm, strand][(_[0], _[1])] for _ in IN[key][qidx, :]])
        assert np.all(idxs == sorted(idxs))
