...
def _get_read_num(files, verbose=False):
    ...
    for f_i, fn in enumerate(files):
        ...
        bash_command = "samtools idxstats %s | grep -v 'GL00' | cut -f 3 | tr '\\n' '+' | sed 's/.$/\\n/g'| bc" % (fn)
        cur_read_cnt = int(subprocess.check_output(bash_command, shell=True))
        if cur_read_cnt <= 0:
            print("BAM File not indexed or no mapped reads found", file=sys.stderr)
            read_num[f_i] = 1 
        else:
            read_num[f_i] = cur_read_cnt
    return read_num 
...
def cov_from_bam(chrm, start, stop, files, subsample = 0, verbose = False,
                 ...
                 grid=False, strand=None, highlight=None, highlight_color='magenta', highlight_label=None, rpm_ind=False,
    ...
    ### read per million plotting  
    if rpm_ind:
        scale_factor = _get_read_num(files)
        counts = (counts / scale_factor) * 1e6
    ...
    if log:
        ...
        ax.set_ylabel('Reads per Million Mapped') 
    else:
