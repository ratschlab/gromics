...
def parse_options(argv):
    ...
    parser = ArgumentParser(prog='compute_lib_size',
                            description='This script collects expression counts in TSV format and aggregates them in a single HDF5 file. Please note \
                                         ...
                                         -i, -f, -p.\n')
    parser.add_argument('-i', '--input', dest='infiles_fnames', metavar='STR', help='list of expression count files in TSV format', default='-')
...
def main():
    ...
    for f, fname in enumerate(files):
        ...
        if f == 0:
            ...
            OUT.create_dataset('gids', data=data[:, 0].view(sp.chararray).encode('utf-8'))
            OUT.create_dataset('counts', data=data[:, 1][:, sp.newaxis].astype('int'), chunks=True, compression='gzip', maxshape=(data.shape[0], None))
        else:
            assert(sp.all(OUT['gids'][:].view(sp.chararray).decode('utf-8') == data[:, 0]))
